...
public class Main extends JFrame {
	...
	/**
	 * Chargement des niveaux avec un JFileChooser vers la zone d'animation physique
	 */
	// Kitimir Yim
	private void chargerNiveauVersZoneAnimationPhysique() {

		JFileChooser selecteurFichier = new JFileChooser();
		selecteurFichier.setCurrentDirectory(Sauvegarder.getCheminSauvegarde().toFile());
		selecteurFichier.setFileFilter(
				new javax.swing.filechooser.FileNameExtensionFilter("Fichiers de niveau (*.dat)", "dat"));

		int resultat = selecteurFichier.showOpenDialog(Main.this);

		if (resultat == JFileChooser.APPROVE_OPTION) {

			File fichierChoisi = selecteurFichier.getSelectedFile();
			String nomFichierNiveau = fichierChoisi.getName();
			Niveau niveau = Sauvegarder.chargerNiveauMesTrucs(nomFichierNiveau);
