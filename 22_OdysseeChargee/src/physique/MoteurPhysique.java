...
//			System.out.println("Point quelconque : " +segment.getPointQuelconque() + "\n");
	...
	private static Vecteur2D calculVitesseApresCollisionFaceLateraleSegment(Vaisseau vaisseau, Segment segment) {
		try {
			...
			orientationDistanceSegmentVaisseau = vaisseau.getPosition().soustrait(segment.getPointQuelconque())
					.normalise();
			...
			Vecteur2D lieuCollision = segment.getExtremiteA()
					.additionne(segment.getAxe().multiplie(dVaisseauExtrASurAxe)); // surveiller si additionne pu
			...
			if (distanceSegmentVaisseau.prodScalaire(segment.getNormale()) > 0) {
				vaisseau.setPosition(
						lieuCollision.additionne(segment.getNormale().multiplie(vaisseau.getRayon() + EPSILON)));
			} else {
				vaisseau.setPosition(
						lieuCollision.additionne(segment.getNormale().multiplie(-1 * (vaisseau.getRayon() + EPSILON))));
			}
